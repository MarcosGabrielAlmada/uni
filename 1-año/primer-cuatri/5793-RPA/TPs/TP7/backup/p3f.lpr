...
function repite(n, d: integer): integer;
begin
  if (n < 10)
  then
    begin
  ...
  begin
    if (n mod 10 = d)
    then repite := d*11 + 100*repite(n div 10, d)
    else repite := n mod 10 + 10*repite(n div 10, d);
  end;
...
begin
  ...
  write(n, ' con los digitos ', d, ' repetidos, queda: ', repite(n,d));
  readln();
...
REPITE D en N
Case Base
* Si N tiene un dígito y es igual al D, REPITE D en N es igual a N duplicado
* Si N tiene un dígito y es distinto al D, REPITE D en N es igual a N

Caso General
* Si N tiene más de un dígito y su último dígito es igual al D,
REPITE d en N es igual a REPITE D en N',más el  dígito D duplicado

* Si N tiene más de un dígito y su último dígito es distinto al D,
REPITE d en N es igual a REPITE D en N' con el último dígito de N al final

(Siendo N': N sin el último dígito)
