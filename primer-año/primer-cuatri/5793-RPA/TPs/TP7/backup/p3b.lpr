...
function masSignificativo(n, d: integer): boolean
begin
  while (n >= 10) do
  begin
    n := n div 10;
  ...
  if (n = d)
  then masSignificativo := true
  else masSignificativo := false;
end;
...
begin
  ...
  if (masSignificativo(n,d))
  then
    writeln(d, ' SI es el mas significativo de ', n)
  else
    writeln(d, ' NO es el mas significativo de ', n);
  readln();
